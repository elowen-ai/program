...
#[derive(Accounts)]
pub struct WithdrawEdaUSDC<'info> {
    #[account(
        mut,
    ...
    pub eda_usdc_ata: Account<'info, TokenAccount>,
...
pub fn withdraw(ctx: Context<WithdrawEdaUSDC>, amount: u64) -> Result<()> {
    let eda_usdc_ata = &ctx.accounts.eda_usdc_ata;

    require!(
        eda_usdc_ata.amount >= amount,
        CustomError::NotEnoughBalanceInVault
    );

    let sender_account = &eda_usdc_ata.to_account_info();
